    ...
    <>
      ...
      <section id="featured" className="py-20 md:py-28" style={{ backgroundColor: "#0d0509" }}>
        <Container>
          ...
          <ScrollReveal stagger className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {featuredProducts.map((product) => (
              <Link key={product.slug} href={`/products/${product.slug}`} className="group block">
                <div className="relative aspect-[4/5] overflow-hidden" style={{ backgroundColor: "#180a12", borderRadius: 12 }}>
                  <img src={product.image} alt={product.name} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100" style={{ background: "linear-gradient(to top, rgba(13,5,9,0.6), transparent)" }} />
                  <div className="absolute bottom-0 left-0 right-0 p-6 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <span className="inline-block px-6 py-2 text-xs font-semibold uppercase tracking-wider text-white" style={{ backgroundColor: "#ff2e88", borderRadius: 9999 }}>Quick Add</span>
                  </div>
                </div>
                <div className="mt-4">
          ...
          <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-2 md:gap-20">
            <ScrollReveal>
              <div>
                ...
                <div className="mt-8">
                  <Link href="/products" className="inline-block text-sm font-semibold uppercase tracking-wider text-white transition-all hover:opacity-90" style={{ backgroundColor: "#ff2e88", padding: "14px 32px", borderRadius: 9999 }}>
                    Shop Now
                  </Link>
                </div>
              </div>
            ...
            <ScrollReveal>
              <div className="relative aspect-[4/5] overflow-hidden" style={{ backgroundColor: "#22101a", borderRadius: 12 }}>
      ...
      <section className="py-12" style={{ borderTop: "1px solid #3d1e2c", borderBottom: "1px solid #3d1e2c", backgroundColor: "#0d0509" }}>
        <Container>
          <ScrollReveal stagger className="grid grid-cols-1 gap-8 text-center md:grid-cols-3">
            <div>
        ...
        <Container className="max-w-xl text-center">
          <ScrollReveal>
            ...
            <form className="mt-8 flex flex-col gap-3 sm:flex-row" onSubmit={(e) => e.preventDefault()}>
              <input type="email" placeholder="Enter your email" className="flex-1 px-5 py-4 text-sm outline-none" style={{ border: "1px solid #3d1e2c", color: "#f8eef3", backgroundColor: "#180a12", borderRadius: 9999 }} />
              <button type="submit" className="text-sm font-semibold uppercase tracking-wider transition-all hover:opacity-90" style={{ backgroundColor: "#ff2e88", color: "#f8eef3", padding: "14px 32px", borderRadius: 9999 }}>
                Subscribe
              </button>
            </form>
          </ScrollReveal>
